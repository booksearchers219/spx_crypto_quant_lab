...
def robust_fetch_data(ticker, period="60d", interval="1h", max_retries=3):
        ...
        try:
            ...
            if data is None or len(data) < 100:
                if attempt < max_retries - 1:
                    ...
                    continue
                return None
            print(f"✅ Fetched {len(data)} {interval} bars for {ticker}")
            return data
...
def run_equity_cycle(reset=False):
    ...
    logger.info(f"Using {len(active_tickers)} equity tickers: {active_tickers}")
    ...
    for ticker in active_tickers[:15]:
        try:
            ...
            short_ma_value = float(df['short_ma'].iloc[-1]) if 'short_ma' in df.columns and pd.notna(df['short_ma'].iloc[-1]) else None
            ...
            print(f"DEBUG {ticker:6} | sig:{signal} | RSI:{rsi_value:.1f} | Price:{close_value:.2f} | ShortMA:{short_ma_str}")
    ...
    print(f"\n💰 Equity Portfolio Summary (Aggressive + Rebalance)")
...
if __name__ == "__main__":
    ...
    print("🚀 Starting Equity Bot (Aggressive + Rebalance Mode)")
